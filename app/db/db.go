package db

import (
	"context"
	"database/sql"
	"fmt"
	"log"
	"os"
	"time"

	"github.com/anthdm/superkit/db"
	"gorm.io/driver/postgres"

	_ "github.com/lib/pq"
	_ "github.com/mattn/go-sqlite3"

	"gorm.io/driver/sqlite"
	"gorm.io/gorm"
)

// By default this is a pre-configured Gorm DB instance.
// Change this type based on the database package of your likings.
var dbInstance *gorm.DB

// Get returns the instantiated DB instance.
func Get() *gorm.DB {
	return dbInstance
}
func openDB() (*sql.DB, error) {
	db, err := sql.Open("postgres", "postgres://burstman:maison123@localhost:5432/task_manager_db")
	if err != nil {
		fmt.Println("error connecting to database")
		return nil, err
	}
	db.SetMaxOpenConns(25)
	db.SetMaxIdleConns(25)
	duration, err := time.ParseDuration("15m")
	if err != nil {
		return nil, err
	}

	db.SetConnMaxIdleTime(duration)

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	err = db.PingContext(ctx)
	if err != nil {
		return nil, err
	}
	return db, nil
}
func init() {
	// Create a default *sql.DB exposed by the superkit/db package
	// based on the given configuration.
	config := db.Config{
		Driver:   os.Getenv("DB_DRIVER"),
		Name:     os.Getenv("DB_NAME"),
		Password: os.Getenv("DB_PASSWORD"),
		User:     os.Getenv("DB_USER"),
		Host:     os.Getenv("DB_HOST"),
	}
	// Based on the driver create the corresponding DB instance.
	// By default, the SuperKit boilerplate comes with a pre-configured
	// ORM called Gorm. https://gorm.io.
	//
	// You can change this to any other DB interaction tool
	// of your liking. EG:
	// - uptrace bun -> https://bun.uptrace.dev
	// - SQLC -> https://github.com/sqlc-dev/sqlc
	// - gojet -> https://github.com/go-jet/jet

	switch config.Driver {
	case db.DriverSqlite3:
		dbinst, err := db.NewSQL(config)
		if err != nil {
			fmt.Println("failed to create sqlite db")
			log.Fatal(err)
		}
		defer dbinst.Close()
		dbInstance, err = gorm.Open(sqlite.New(sqlite.Config{
			Conn: dbinst,
		}))
		if err != nil {
			fmt.Println("failed to create sqlite db")
			log.Fatal(err)
		}
	case "postgres":

		postgresDb, err := openDB()
		if err != nil {
			fmt.Println("failed to create postgres db")
			log.Fatal(err)
		}

		dbInstance, err = gorm.Open(postgres.New(postgres.Config{
			Conn: postgresDb,
		}))
		if err != nil {
			fmt.Println("failed to create postgres db")
			log.Fatal(err)
		}
	default:
		log.Fatal("invalid driver:", config.Driver)
	}
	fmt.Println(dbInstance.Name())
}
